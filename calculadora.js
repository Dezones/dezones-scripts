(function() {
    ...
    <div id="tw_calc_container" style="position:fixed; top:60px; right:20px; z-index:9999; background:#1a0b2e; padding:20px; border:2px solid #00ffff; border-radius:12px; width:350px; color:#e0d5ff; font-family:'Segoe UI', Tahoma, sans-serif; box-shadow: 0 0 20px rgba(0, 255, 255, 0.3); border-top: 8px solid #00ffff;">
        ...
        <div style="margin-top:15px;">
            ...
            <label style="font-size:11px; color:#b19cd9; font-weight:bold;">MOEDAS JÁ GUARDADAS (Academia)</label>
            ...
            <label style="font-size:11px; color:#b19cd9; font-weight:bold;">QUANTOS NOBRES QUER PRODUZIR?</label>
            <input type="number" id="calc_nobres_produzir" value="100" style="width:100%; background:#2d1b4d; border:1px solid #00ffff; color:#fff; padding:8px; border-radius:5px; margin-bottom:12px; outline:none;">

            <label style="font-size:11px; color:#b19cd9; font-weight:bold;">DESCONTO BANDEIRA (%)</label>
            <input type="number" id="calc_desc" value="48" style="width:100%; background:#2d1b4d; border:1px solid #00ffff; color:#fff; padding:8px; border-radius:5px; margin-bottom:20px; outline:none;">
        ...
        <div id="tw_res" style="margin-top:20px; font-size:13px; display:none; background:rgba(0, 255, 255, 0.05); padding:15px; border-radius:8px; border:1px dashed #00ffff;">
            <div style="margin-bottom:8px;">Limite Final: <span id="res_limite_final" style="color:#00ffff; font-weight:bold; float:right;"></span></div>
    ...
    document.getElementById('btn_calc_tw').onclick = function() {
        ...
        const aProduzir = parseInt(document.getElementById('calc_nobres_produzir').value);
        const desc = (100 - parseFloat(document.getElementById('calc_desc').value)) / 100;

        const limiteFinal = limiteAtual + aProduzir;
        let moedasNecessarias = 0;

        // Soma o custo de cada novo nobre (progressão)
        for (let i = limiteAtual + 1; i <= limiteFinal; i++) {
            moedasNecessarias += i;
        }

        const faltam = Math.max(0, moedasNecessarias - guardadas);

        document.getElementById('res_limite_final').innerText = limiteFinal;
